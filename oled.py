...
# Boot Screen and timer
  ...
  draw.text((x, top),      "    piHome Display",  font=font, fill=255)
  ...
  while True:
    ...
    if mode==2:
      # Get system data
      connectedClients=[]
      cmd = "cd ~/ovpns; ls *.ovpn | tr -d '\n' | sed 's/.ovpn/ /g'"
      clientNames = subprocess.check_output(cmd, shell = True )
      clientNames = str.strip(clientNames.decode('UTF-8'))
      clientArray = clientNames.split(' ')
      for i in range(len(clientArray)):
        cmd = "sudo cat /var/log/openvpn-status.log | grep -q " + str(clientArray[i])
        result = subprocess.run(cmd, shell = True )
        if result.returncode==0:
          connectedClients.append(clientArray[i])

      # If any clients are connected, display them - skip screen if not
      if len(connectedClients) != 0:
        # Draw a black filled box to clear the image.
        draw_black_box()
        
        # Display system stats    
        draw.text((x, top),         "     PiVPN Data",  font=font, fill=255)
        draw.text((x, top+14),      "Currently Connected:",  font=font, fill=255)
        dHeight = top+24
        for i in range(len(connectedClients)): 
          print (connectedClients[i])
          draw.text((x, dHeight),    connectedClients[i], font=font, fill=255)
          dHeight = dHeight+10
        # Display image.
        draw_image_on_display()
        time.sleep(screen_delay)
